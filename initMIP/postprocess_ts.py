...
parser.add_argument(
    "-e", "--experiment", dest="experiment", choices=["ctrl", "asmb"], help="Output size type", default="ctrl"
)
...
if __name__ == "__main__":
    ...
    cmd = ["ncks", "-O", "-d", "time,9,-1", "-v", "{}".format(",".join(pism_copy_vars)), infile, out_file]
    sub.call(cmd)
    ...
    if EXP in ("ctrl"):
        ...
        ncks_cmd = ["ncks", "-O", "-4", "-L", "3", "-d", "time,1", out_file, init_file]
        sub.call(ncks_cmd)
