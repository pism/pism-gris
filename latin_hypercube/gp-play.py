...
if __name__ == "__main__":
    ...
    parser.add_argument("--test", dest="test", help="Test code", action="store_true", default=False)
    ...
    if options.test:
        ...
        with Pool(n_procs) as pool:
            results = pool.map(
                partial(year, samples_file=samples_file, X_new=X_new, metadata=metadata),
    ...
    m_gp = dict()
    m_gp["years"] = np.asarray([item["year"] for item in results])
    m_lhs = dict()
    m_lhs["years"] = np.asarray([item["year"] for item in results])

    for idx, pctl in enumerate(m_percentiles):
        m_gp[pctl] = np.squeeze(np.asarray([item["gp"].values[idx, :] for item in results]).reshape(1, -1))
        m_lhs[pctl] = np.squeeze(np.asarray([item["lhs"].values[idx, :] for item in results]).reshape(1, -1))
