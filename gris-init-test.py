...
for n, combination in enumerate(combinations):
    ...
    script = 'init_{domain}_g{grid}m_{system}_{queue}.sh'.format(domain=domain.lower(), grid=grid, system=system, queue=queue)
    ...
    with open(script, 'w') as f:
        ...
        outfile = '{domain}_g{grid}m_test_{system}_{queue}.nc'.format(domain=domain.lower(),grid=grid, system=system, queue=queue)
        ...
        cmd = ' '.join([batch_system['mpido'], prefix, all_params, '2>&1 | tee job{exp}.${batch}'.format(exp=exp_type, batch=batch_system['job_id'])])
