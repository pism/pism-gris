...
grid_choices = [1500, 1200, 900, 600, 450]

# set up the option parser
parser = ArgumentParser()
parser.description = "Generating scripts for model initialization."
parser.add_argument("--start_file", dest="in_file",
...
for n, combination in enumerate(combinations):
    ...
    full_exp_name = '_'.join(['hc_refine', climate, vversion, bed_type, '_'.join(
    ...
    with open(script, 'w') as f:
        ...
        outfile = '{domain}_g{grid}m_hc_refine_{experiment}.nc'.format(
        ...
        if grid_mapping[grid] > 0:
            ...
            regridfile = os.path.join(odir, 'save_{domain}_g{grid}m_hc_refine_{experiment}_{start}.000.nc'.format(
                domain=domain.lower(), grid=previous_grid, experiment=experiment, start=start))
