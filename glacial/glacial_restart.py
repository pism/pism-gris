...
parser.add_argument("--climate", dest="climate", choices=["abrupt_glacial"], help="Climate", default="abrupt_glacial")
parser.add_argument(
    "--calving",
    dest="calving",
...
parser.add_argument(
    ...
    help="Ocean coupler",
    default="abrupt_glacial",
)
parser.add_argument(
    "--ocean_melt",
    dest="ocean_melt",
    choices=["x", "10myr_latitudinal", "20myr_latitudinal"],
    help="Ocean melt type",
    default="20myr_latitudinal",
)
parser.add_argument(
    "-d", "--domain", dest="domain", choices=["gris", "gris_ext"], help="sets the modeling domain", default="gris_ext"
)
parser.add_argument("--exstep", dest="exstep", type=int, help="Writing interval for spatial time series", default=100)
parser.add_argument(
    "-f",
    "--o_format",
    dest="oformat",
...
parser.add_argument(
    "--precip", dest="precip", choices=["racmo", "hirham"], help="Precipitation model", default="racmo"
)
parser.add_argument(
    "--stable_gl",
...
paleo_start_year = 0
paleo_end_year = 27500
restart_step = 2500

for n, combination in enumerate(combinations):

    eigen_calving_k, fice, fsnow, backpressure_max, ocean_melt_power, thickness_calving_threshold, ppq, tefo, phi_min, phi_max, topg_min, topg_max = (
        combination
    )
        ...
        for start in range(paleo_start_year, paleo_end_year, restart_step):
            ...
            with open(script, "w") as f:
                ...
                ocean_params_dict = generate_ocean(
                    ...
                    ocean_delta_SL_file="pism_abrupt_glacial_climate_forcing.nc",
                    ocean_frac_mass_flux_file="pism_abrupt_glacial_climate_forcing.nc",
                ...
                scalar_ts_dict = generate_scalar_ts(
                    outfile, tsstep, start=paleo_start_year, end=paleo_end_year, odir=odir
                )
    ...
    with open(script_post, "w") as f:
        ...
        myfiles = " ".join(
            ["{}_{}.000.nc".format(extra_file, k) for k in range(paleo_start_year + exstep, paleo_end_year, exstep)]
        )
        ...
        ts_file = os.path.join(
            odir,
            "ts_{domain}_g{grid}m_straight_{experiment}".format(
                domain=domain.lower(), grid=grid, experiment=full_exp_name
            ),
        )
        myfiles = " ".join(
            [
                "{}_{}_{}.nc".format(ts_file, k, k + restart_step)
                for k in range(paleo_start_year, paleo_end_year, restart_step)
            ]
        )
