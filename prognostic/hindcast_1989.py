...
parser.add_argument(
    "-a", "--start_date", dest="start_date", help="""Start date in ISO format. Default=2000-1-1""", default="2000-1-1"
)
parser.add_argument(
    "-e", "--end_date", dest="end_date", help="""End date in ISO format. Default=2008-1-1""", default="2008-1-1"
)
parser.add_argument(
...
parser.add_argument(
    "-q", "--queue", dest="queue", choices=list_queues(), help="""queue. default=t1standard.""", default="t1standard"
)
parser.add_argument(
    "--climate", dest="climate", choices=["const", "pdd", "pdd_lapse", "flux"], help="Climate", default="pdd_lapse"
)
parser.add_argument(
    "--calving",
    dest="calving",
    choices=["float_kill", "ocean_kill", "eigen_calving", "thickness_calving", "vonmises_calving"],
    help="claving",
    default="vonmises_calving",
)
parser.add_argument("--ocean", dest="ocean", choices=["given", "given_mbp"], help="ocean", default="given")
parser.add_argument(
    "-d",
    "--domain",
    dest="domain",
...
parser.add_argument(
    "-g", "--grid", dest="grid", type=int, choices=grid_choices, help="horizontal grid resolution", default=600
)
parser.add_argument("--o_dir", dest="odir", help="output directory. Default: current directory", default=".")
parser.add_argument(
    "--o_size", dest="osize", choices=["small", "medium", "big", "big_2d"], help="output size type", default="big_2d"
)
parser.add_argument(
    "-s", "--system", dest="system", choices=list_systems(), help="computer system to use.", default="pacman"
)
parser.add_argument(
    "-b", "--bed_type", dest="bed_type", choices=list_bed_types(), help="output size type", default="cresisp"
)
parser.add_argument(
    "--bed_deformation",
    dest="bed_deformation",
    choices=[None, "lc", "iso"],
    help="Bed deformation model.",
    default=None,
)
parser.add_argument(
...
parser.add_argument(
    "--hydrology", dest="hydrology", choices=["null", "diffuse"], help="Basal hydrology model.", default="diffuse"
)
parser.add_argument(
    "--melange_backpressure",
    dest="melange_backpressure",
    action="store_true",
    help="Melange backpressure model.",
    default=False,
)
parser.add_argument(
    "--regrid_thickness",
    ...
    action="store_true",
    help="Regrid ice thickness from input file rather than from boot file",
    default=False,
)
parser.add_argument(
    "--stress_balance",
...
parser.add_argument(
    "--dataset_version", dest="version", choices=["2"], help="input data set version", default="2_1985"
)
parser.add_argument(
...
try:
    ...
    pass
cmd = ["create_timeline.py", "-a", start_date, "-e", end_date, "-d", "2000-01-01", pism_timefile]
sub.call(cmd)
...
for n, combination in enumerate(combinations):
    ...
    experiment = "_".join(
        [climate, vversion, bed_type, "_".join(["_".join([k, str(v)]) for k, v in list(name_options.items())])]
    )

    script = "hc_{}_g{}m_{}.sh".format(domain.lower(), grid, experiment)
    scripts.append(script)
    script_post = "hc_{}_g{}m_{}_post.sh".format(domain.lower(), grid, experiment)
    ...
    with open(script, "w") as f:
        ...
        outfile = "{domain}_g{grid}m_{experiment}_{start}_{end}.nc".format(
            domain=domain.lower(), grid=grid, experiment=experiment, start=start_date, end=end_date
        )
        ...
        if climate in ("flux"):
            climate_params_dict = generate_climate(
                climate, surface_given_file=atmosphere_file, force_to_thickness_file=pism_dataname
            )
        else:
            climate_params_dict = generate_climate(
                climate,
        ...
        ocean_params_dict = generate_ocean(
            ocean,
            ocean_given_file="ocean_forcing_1200m_1989-2011_v2_1985_ctrl_const_ctrl.nc",
