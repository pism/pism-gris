...
def prepare_pr(dry=False):
    ...
    if dry == False:
        ...
        opt = [c.Atted(mode="o", att_name="units", var_name="precipitation", value="kg m-2 day-1"),
...
def prepare_smb(dry=False):
    ...
    if dry == False:
        ...
        logger.info('calculate time mean')
        cdo.ydaymean(input='-mulc,' + str(ice_density / 1000) + ' -del29feb ' + smb_merged_file_daily_mean, output=smb_merged_file_time_mean)
        ...
        opt = [c.Atted(mode="o", att_name="units", var_name="climatic_mass_balance", value="kg m-2 day-1"),
...
for grid_spacing in (18000, 9000, 4500, 3600, 3000, 2400, 1800, 1500, 1200, 900, 600):
    ...
    epsg3413_merged_file_time_mean = 'DMI-HIRHAM5_GL2_ERAI_2001_2014_{}_BIL_EPSG3413_{}m.nc'.format(time_mean, grid_spacing)
    logger.info('remapping {} to {}'.format(merged_file_time_mean, epsg3413_merged_file_time_mean))
    tmpfile = cdo.remapbil('{} -setgrid,{}'.format(grid_file, rotated_grid_file),
    ...
    opt = [c.Atted(mode="o", att_name="units", var_name="time", value="day"),
