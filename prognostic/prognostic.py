...
parser.add_argument(
    "--calving",
    dest="calving",
    choices=["float_kill", "ocean_kill", "eigen_calving", "thickness_calving", "stress_calving"],
    help="claving",
...
parser.add_argument("--duration", dest="dura", type=int, help="Length of simulation in years (integers)", default=100)
parser.add_argument(
    "-f",
    "--o_format",
    dest="oformat",
...
for n, combination in enumerate(combinations):
    ...
    with open(script, "w") as f:
        ...
        outfile = "{domain}_g{grid}m_{experiment}_{dura}a.nc".format(
            domain=domain.lower(), grid=grid, experiment=experiment, dura=dura
        )
        ...
        ocean_params_dict = generate_ocean("given", ocean_given_file="ocean_forcing_latitudinal_285.nc")
...
submit = "submit_{domain}_g{grid}m_{climate}_{bed_type}.sh".format(
    domain=domain.lower(), grid=grid, climate=climate, bed_type=bed_type
)
try:
    os.remove(submit)
except OSError:
