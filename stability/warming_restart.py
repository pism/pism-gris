...
for n, combination in enumerate(combinations):
    ...
    with open(script_post, 'w') as f:
        ...
        f.write(cmd)
        flux_file = os.path.join(odir, scalar_dir, 'surface_flux_ts_{domain}_g{grid}m_{experiment}'.format(domain=domain.lower(), grid=grid, experiment=full_exp_name))
        cmd = ' '.join(['cdo fldsum', myoutfile, flux_file])
        f.write(cmd)
