...
for n, combination in enumerate(combinations):
    ...
    experiment =  '_'.join(['thermo', vversion, bed_type, '_'.join(['_'.join([k, str(v)]) for k, v in name_options.items()])])
    ...
    with open(script, 'w') as f:
        ...
        outfile = '{domain}_g{grid}m_thermo_{experiment}_100ka.nc'.format(domain=domain.lower(),grid=grid, experiment=experiment)
