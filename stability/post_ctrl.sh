...
for rcp in 26 45 85; do
    for year in 3000; do
        for basin in CW NE NO NW SE SW; do
            for run in CTRL; do
                cdo -L  setattribute,limnsw@units="cm" -setattribute,long_mame="contribution to global mean sea level" -divc,365 -divc,-1e13 -selvar,limnsw -sub -selyear,$year $odir/basins/scalar/ts_b_${basin}_ex_gris_g${grid}m_v3a_rcp_${rcp}_id_${run}_0_1000.nc -selyear,2008 $odir/basins/scalar/ts_b_${basin}_ex_gris_g${grid}m_v3a_rcp_${rcp}_id_${run}_0_1000.nc $odir/basins/dgmsl/dgmsl_b_${basin}_g${grid}m_rcp_${rcp}_${run}_${year}.nc
            done
        done
    done
done
...
mkdir -p $odir/final_states
cd $odir/state
for file in gris_g${grid}m*_0_1000.nc; do
    gdal_translate NETCDF:../final_states/$file:thk ../final_states/thk_$file.tif
done
cd ../../
