...
for n, combination in enumerate(combinations):
    ...
    if bed_type is None:
        experiment = "_".join([vversion, "_".join(["_".join([k, str(v)]) for k, v in list(name_options.items())])])
    else:
        experiment = "_".join(
    ...
    with open(script, "w") as f:
        ...
        outfile = "{domain}_g{grid}m_{experiment}_{dura}a_{exp}.nc".format(
            domain=domain.lower(), grid=grid, experiment=experiment, dura=end, exp=exp
        )
